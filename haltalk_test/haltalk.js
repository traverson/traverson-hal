/*
 * This test suite runs Traverson and traverson-hal against the haltalk server
 * at http://haltalk.herokuapp.com. It is not included in the standard test
 * suite. Use bin/haltalk_test.sh to run it.
 */
...
describe('Traverson (when tested against the haltalk server at ' + rootUri +
  ...
  it('should follow a multi-element path with templating and embedded docs',
    ...
    waitFor(
      ...
      function() {
        var resultDoc = checkResponseWithBody();
        resultDoc.content.should.contain('Awesome!');
        done();
      }
    );
  });
